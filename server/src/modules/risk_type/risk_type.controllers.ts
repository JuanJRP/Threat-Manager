...
export class RiskTypeController {
  ...
  async updateRiskTypeById(req: Request, res: Response): Promise<void> {
    try {
      const updatedRiskType = await riskTypeService.updateRiskType(Number(req.params.id), req.body);
      res.status(200).json(updatedRiskType);
    } catch (error) {
      res.status(500).json({ message: 'Error al actualizar el tipo de riesgo' });
    }
  }
